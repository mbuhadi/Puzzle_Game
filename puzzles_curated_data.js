...
function studioScreenHTML(sc, board) {
  ...
  const sprites = sc.sprites.map(s => {
    ...
    return `<div style="position:absolute; left:${pct(s.bx / W * 100)}; top:${pct(s.by / H * 100)};` +
      ` width:${pct(s.bw / W * 100)}; height:${pct(s.bh / H * 100)};` +
      ` background:url('${encodeURI(s.src)}') ${pct(px)} ${pct(py)} / ${pct(s.iw / s.w * 100)} ${pct(s.ih / s.h * 100)} no-repeat;` +
